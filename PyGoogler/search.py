...
class GoogleSearch(object):
    ...
    def _extract_result(self, result):
        ...
        if not url:  # not title or : # or not desc:
            return None
    ...
    def _extract_description(self, result):
        # XGoogle doesn't handle this function well, so I decided to re-write it using html2text
        # ret = result.getPlainTextContent()
        # ret =html2text.html2text(ret)
        # return ret
        ...
        def looper(tag):
            ...
            for t in tag:
                ...
                try:
                    if t.name != 'span' or t.get('class') != 'st':
                        continue
